...
def post_process_text(text: str) -> str:
    ...
    if rows and all(r[1] for r in rows):
        ...
        tokens = [line for line in lines[1:] if line not in {"日期", "考试科目", "考试时间"}]
        i = 0
...
def run_inference(image_path: str) -> str:
    ...
    model = Qwen3VLForConditionalGeneration.from_pretrained(
        "/home/ubun/robot/Florence2Semantic/models/qwen3-vl-2B-Instruct",
        dtype="auto",
        device_map="auto",
    )
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": "只识别显示器上的文字内容。要求：1)使用纯文本，不要使用markdown格式、分隔符等特殊字符",
                },
            ],
        }
    ]
    ...
    return output_text[0] if isinstance(output_text, list) and output_text else ""


def demo_post_process() -> None:
    raw_text = (
        "苏州市盲聋学校期末考试安排\n\n"
        "日期 考试科目 考试时间\n"
        "1.27 语文 9:00-11:00\n"
...
if __name__ == "__main__":
    if os.environ.get("POSTPROCESS_DEMO") == "1":
        demo_post_process()
    else:
        # 展开 ~ 路径为绝对路径
        image_path = os.path.expanduser("~/1.jpeg")
        raw_text = run_inference(image_path)
        print(raw_text)
        print(post_process_text(raw_text))
